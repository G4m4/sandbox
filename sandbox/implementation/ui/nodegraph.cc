/// @file nodegraph.cc
/// @brief SandBox ImGui nodegraph implementation
/// @author gm
/// @copyright gm 2019
///
...
struct _IgnoreSlot {
  ...
  /// Slot kind. Not actual slot kind, but +1 or -1. Used to determine if slot
  /// is input or output.
  int slot_kind = 0;
...
struct _CanvasStateImpl {
  ...
  /// Node id of node that should be selected on next frame, while deselecting
  /// any other nodes.
  ...
  /// Previous canvas pointer. Used to restore proper gCanvas value when nesting
  /// canvases.
  CanvasState *prev_canvas = nullptr;
  /// A list of node/slot combos that can not connect to current pending
  /// connection.
...
ImU32 MakeSlotDataID(const char *data, const char *slot_title, void *node_id,
                     bool input_slot) {
  ImU32 slot_id =
      ImHashStr(slot_title, 0, ImHashData(&node_id, sizeof(node_id)));
  if (input_slot) {
...
float GetDistanceToLineSquared(const ImVec2 &point, const ImVec2 &a,
                               const ImVec2 &b) {
  float tx, ty, t, u;
  tx = b.x - a.x;
  ty = b.y - a.y;
...
bool RenderConnection(const ImVec2 &input_pos, const ImVec2 &output_pos,
                      float thickness) {
  ...
  // Check each segment and determine if mouse is hovering curve that is to be
  // drawn
  float min_square_distance = FLT_MAX;
  for (int i = 0; i < draw_list->_Path.size() - 1; i++) {
    min_square_distance = ImMin(
        min_square_distance,
        GetDistanceToLineSquared(ImGui::GetMousePos(), draw_list->_Path[i],
                                 draw_list->_Path[i + 1]));
  }

  // Draw curve, change when it is hovered
  bool is_close = min_square_distance <= thickness * thickness;
  draw_list->PathStroke(is_close ? canvas->colors[ColConnectionActive]
...
void BeginCanvas(CanvasState *canvas) {
  ...
  if (!ImGui::IsMouseDown(0) && ImGui::IsWindowHovered()) {
    ...
    if (!io.KeyShift && io.KeyCtrl) {
      if (io.MouseWheel != 0)
        canvas->zoom = ImClamp(
            canvas->zoom + io.MouseWheel * canvas->zoom / 16.f, 0.3f, 3.f);
...
void EndCanvas() {
  ...
  if (const ImGuiPayload *payload = ImGui::GetDragDropPayload()) {
    ...
    if (strncmp(payload->DataType, data_type_fragment,
                sizeof(data_type_fragment) - 1) == 0) {
      auto *drag_data = (_DragConnectionPayload *)payload->Data;
      ImVec2 slot_pos{
          impl->cached_data.GetFloat(
              MakeSlotDataID("x", drag_data->slot_title, drag_data->node_id,
                             IsInputSlotKind(drag_data->slot_kind))),
          impl->cached_data.GetFloat(
              MakeSlotDataID("y", drag_data->slot_title, drag_data->node_id,
                             IsInputSlotKind(drag_data->slot_kind))),
      };
  ...
  case State_None: {
    ...
    if (ImGui::IsMouseDown(0) &&
        ImGui::GetCurrentWindow()->ContentRegionRect.Contains(
            ImGui::GetMousePos())) {
      if (ImGui::IsWindowHovered()) {
        ...
        if (!ImGui::IsAnyItemActive()) {
          ...
          if (!io.KeyCtrl && !io.KeyShift) {
            ...
            impl->do_selections_frame =
                ImGui::GetCurrentContext()->FrameCount + 1;
          }
        }
      }
  ...
  case State_Select: {
    if (ImGui::IsMouseDown(0)) {
      draw_list->AddRectFilled(impl->selection_start, ImGui::GetMousePos(),
                               canvas->colors[ColSelectBg]);
      draw_list->AddRect(impl->selection_start, ImGui::GetMousePos(),
                         canvas->colors[ColSelectBorder]);
    } else {
      ImGui::ClearActiveID();
      impl->state = State_None;
    }
...
bool BeginNode(void *node_id, ImVec2 *pos, bool *selected) {
  ...
  if (node_id == impl->auto_position_node_id) {
    // Somewhere out of view so that we dont see node flicker when it will be
    // repositioned
    ImGui::SetCursorScreenPos(ImGui::GetWindowPos() + ImGui::GetWindowSize() +
                              style.WindowPadding);
  } else {
    // Top-let corner of the node
    ImGui::SetCursorScreenPos(ImGui::GetWindowPos() + (*pos) * canvas->zoom +
                              canvas->offset);
  }
...
void EndNode() {
  ...
  ImColor node_color =
      canvas->colors[node_selected ? ColNodeActiveBg : ColNodeBg];
  draw_list->AddRectFilled(node_rect.Min, node_rect.Max, node_color,
                           style.FrameRounding);
  draw_list->AddRect(node_rect.Min, node_rect.Max,
                     canvas->colors[ColNodeBorder], style.FrameRounding);
  ...
  // Save last selection state in case we are about to start dragging multiple
  // selected nodes
  if (ImGui::IsMouseClicked(0)) {
    ImGuiID prev_selected_id = ImHashStr(
        "prev-selected", 0, ImHashData(&impl->node.id, sizeof(impl->node.id)));
  ...
  switch (impl->state) {
  case State_None: {
    // Node selection behavior. Selection can change only when no node is being
    // dragged and connections are not being made.
    if (impl->just_connected || ImGui::GetDragDropPayload() != nullptr) {
    ...
    } else if (impl->do_selections_frame ==
               ImGui::GetCurrentContext()->FrameCount) {
      ...
      // Upon node creation we would like it to be positioned at the center of
      // mouse cursor. This can be done only once widget dimensions are known at
      // the end of rendering and thus on the next frame.
      node_pos = ImGui::GetMousePos() - ImGui::GetCurrentWindow()->Pos -
                 canvas->offset - (node_rect.GetSize() / 2);
  ...
  case State_Drag: {
    if (ImGui::IsMouseDown(0)) {
      // Node dragging behavior. Drag node under mouse and other selected nodes
      // if current node is selected.
      if ((ImGui::IsItemActive() ||
           (impl->drag_node && impl->drag_node_selected && node_selected)))
  ...
  case State_Select: {
    ...
    selection_rect.Min.x =
        ImMin(impl->selection_start.x, ImGui::GetMousePos().x);
    selection_rect.Min.y =
        ImMin(impl->selection_start.y, ImGui::GetMousePos().y);
    selection_rect.Max.x =
        ImMax(impl->selection_start.x, ImGui::GetMousePos().x);
    selection_rect.Max.y =
        ImMax(impl->selection_start.y, ImGui::GetMousePos().y);
...
bool Connection(void *input_node, const char *input_slot, void *output_node,
  ...
  if (input_node == impl->auto_position_node_id ||
      output_node == impl->auto_position_node_id)
    // Do not render connection to newly added output node because node is
    // rendered outside of screen on the first frame and will be repositioned.
    return is_connected;

  ImVec2 input_slot_pos{
      impl->cached_data.GetFloat(
          MakeSlotDataID("x", input_slot, input_node, true)),
      impl->cached_data.GetFloat(
          MakeSlotDataID("y", input_slot, input_node, true)),
  };

  ImVec2 output_slot_pos{
      impl->cached_data.GetFloat(
          MakeSlotDataID("x", output_slot, output_node, false)),
      impl->cached_data.GetFloat(
          MakeSlotDataID("y", output_slot, output_node, false)),
  };
  ...
  bool curve_hovered = RenderConnection(input_slot_pos, output_slot_pos,
                                        canvas->style.curve_thickness);
  if (curve_hovered && ImGui::IsWindowHovered()) {
    if (ImGui::IsMouseDoubleClicked(0))
      is_connected = false;
  }

  impl->cached_data.SetFloat(
      MakeSlotDataID("hovered", input_slot, input_node, true),
      curve_hovered && is_connected);
  impl->cached_data.SetFloat(
      MakeSlotDataID("hovered", output_slot, output_node, false),
      curve_hovered && is_connected);

  void *pending_node_id;
  const char *pending_slot_title;
  int pending_slot_kind;
  if (GetPendingConnection(&pending_node_id, &pending_slot_title,
                           &pending_slot_kind)) {
    _IgnoreSlot ignore_connection{};
    if (IsInputSlotKind(pending_slot_kind)) {
      if (pending_node_id == input_node &&
          strcmp(pending_slot_title, input_slot) == 0) {
      ...
      if (pending_node_id == output_node &&
          strcmp(pending_slot_title, output_slot) == 0) {
...
void EndSlot() {
  ...
  // This here adds extra line between slots because after user renders slot
  // cursor is already past those items. ImGui::ItemSize(slot_rect.GetSize());
  ImGui::ItemAdd(slot_rect, ImGui::GetID(impl->slot.title));

  if (ImGui::IsMouseClicked(0) && ImGui::IsItemHovered())
    ImGui::SetActiveID(ImGui::GetID(impl->slot.title),
  ...
  // Store slot edge positions, curves will connect there
  {
    ...
    impl->cached_data.SetFloat(MakeSlotDataID("x", impl->slot.title,
                                              impl->node.id,
                                              IsInputSlotKind(impl->slot.kind)),
                               x);
    impl->cached_data.SetFloat(MakeSlotDataID("y", impl->slot.title,
                                              impl->node.id,
                                              IsInputSlotKind(impl->slot.kind)),
  ...
  if (ImGui::BeginDragDropSource()) {
    ...
    char drag_id[32];
    snprintf(drag_id, sizeof(drag_id), "new-node-connection-%08X",
             impl->slot.kind);
  ...
  if (IsConnectingCompatibleSlot() && ImGui::BeginDragDropTarget()) {
    ...
    if (auto *payload = ImGui::AcceptDragDropPayload(drag_id)) {
      ...
      // Store info of source slot to be queried by GetConnection()
      if (!IsInputSlotKind(impl->slot.kind)) {
...
bool IsSlotCurveHovered() {
  ...
  int slot_kind;
  if (GetPendingConnection(&node_id, &slot_title, &slot_kind)) {
    // In-progress connection to current slot is hovered
    return node_id == impl->node.id &&
           strcmp(slot_title, impl->slot.title) == 0 &&
           slot_kind == impl->slot.kind;
  }

  // Actual curve is hovered
  return impl->cached_data.GetBool(
      MakeSlotDataID("hovered", impl->slot.title, impl->node.id,
                     IsInputSlotKind(impl->slot.kind)));
}
...
bool IsConnectingCompatibleSlot() {
  ...
  if (auto *payload = ImGui::GetDragDropPayload()) {
    ...
    for (int i = 0; i < impl->ignore_connections.size(); i++) {
      ...
      if (ignored.node_id == impl->node.id &&
          strcmp(ignored.slot_name, ignored.slot_name) == 0 &&
          IsInputSlotKind(ignored.slot_kind) ==
              IsInputSlotKind(impl->slot.kind))
        return false;
    }
